...
def start(config):
    ...
    output_others_entities = []
    for d in client.iter_dialogs():
        #print(d)
        #print(d.name)
        #print(d.entity.id)
        #print(d.entity.access_hash)
        ...
        if d.name in config["output_others_names"] or d.entity.id in config["output_others_ids"]:
            output_others_entities.append(InputChannel(d.entity.id, d.entity.access_hash))
    ...
    async def handler(event):
        for output_channel in output_channel_entities:
            for output_other in output_others_entities:
                result = await client.forward_messages(entity=output_channel.channel_id, messages=event.message)
                if result.fwd_from.from_id is None:
                    pass
                elif result.fwd_from.from_id is not None:
                    if result.fwd_from.from_id.user_id == int(521521130) :
                        await client.forward_messages(entity=output_other.channel_id, messages=event.message)
                    else:
                        pass
